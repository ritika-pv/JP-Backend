...
module.exports ={
    getCategorybySlug:  catchAsyncErrors(async(req,res)=>{
        ...
        const matchedCategory = await Category.find({ 
            $or: [
                { category_name: { $regex: slug } },
                { slug: { $regex: slug} }
             ]
            })
            .populate({path: 'menu_item_id', select: '-createdAt -modifiedAt -__v -deleted'})
            .select('-deleted -createdAt -modifiedAt')
        console.log(matchedCategory,"  ::matched category")
        if(matchedCategory.length==0) return res.status(404).json({success:false ,matchedCategory ,message :'Category not found '})
        else return res.status(200).json({success:true ,matchedCategory ,message :'Category found successfully'})
